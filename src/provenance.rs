...
pub fn build_attestation_envelope(
    ...
    let admission_degraded = matches!(
        admission.outcome.as_str(),
        "warn" | "bypassed" | "disabled"
    );
...
impl BuildProvenance {
    fn from_build_env() -> Self {
        let component = build_env("MCP_PROBE_BUILD_COMPONENT")
            .or_else(|| Some(env!("CARGO_PKG_NAME").to_string()))
            .unwrap_or_else(|| env!("CARGO_PKG_NAME").to_string());
        let server_version = build_env("MCP_PROBE_BUILD_SERVER_VERSION")
            .or_else(|| Some(env!("CARGO_PKG_VERSION").to_string()))
            .unwrap_or_else(|| env!("CARGO_PKG_VERSION").to_string());
        let revision = build_env("MCP_PROBE_BUILD_GIT_SHA").unwrap_or_else(|| UNKNOWN.to_string());
        let reference = build_env("MCP_PROBE_BUILD_GIT_REF").unwrap_or_else(|| UNKNOWN.to_string());
        let dirty = parse_truthy(option_env!("MCP_PROBE_BUILD_GIT_DIRTY"));
        let identity_override = build_env("MCP_PROBE_BUILD_IDENTITY_OVERRIDE");
        ...
        let build_metadata = BuildMetadata {
            profile: build_env("MCP_PROBE_BUILD_PROFILE").unwrap_or_else(|| UNKNOWN.to_string()),
            target: build_env("MCP_PROBE_BUILD_TARGET").unwrap_or_else(|| UNKNOWN.to_string()),
            rustc_version: build_env("MCP_PROBE_BUILD_RUSTC_VERSION")
                .unwrap_or_else(|| UNKNOWN.to_string()),
            source_date_epoch: build_env("MCP_PROBE_BUILD_SOURCE_DATE_EPOCH"),
        };
...
fn option_env_for(key: &str) -> Option<&'static str> {
    match key {
        "MCP_PROBE_BUILD_COMPONENT" => option_env!("MCP_PROBE_BUILD_COMPONENT"),
        "MCP_PROBE_BUILD_SERVER_VERSION" => option_env!("MCP_PROBE_BUILD_SERVER_VERSION"),
        "MCP_PROBE_BUILD_GIT_SHA" => option_env!("MCP_PROBE_BUILD_GIT_SHA"),
        "MCP_PROBE_BUILD_GIT_REF" => option_env!("MCP_PROBE_BUILD_GIT_REF"),
        "MCP_PROBE_BUILD_GIT_DIRTY" => option_env!("MCP_PROBE_BUILD_GIT_DIRTY"),
        "MCP_PROBE_BUILD_PROFILE" => option_env!("MCP_PROBE_BUILD_PROFILE"),
        "MCP_PROBE_BUILD_TARGET" => option_env!("MCP_PROBE_BUILD_TARGET"),
        "MCP_PROBE_BUILD_RUSTC_VERSION" => option_env!("MCP_PROBE_BUILD_RUSTC_VERSION"),
        "MCP_PROBE_BUILD_SOURCE_DATE_EPOCH" => option_env!("MCP_PROBE_BUILD_SOURCE_DATE_EPOCH"),
        "MCP_PROBE_BUILD_IDENTITY_OVERRIDE" => option_env!("MCP_PROBE_BUILD_IDENTITY_OVERRIDE"),
        _ => None,
    }
}
...
mod tests {
    ...
    fn build_identity_appends_dirty_suffix_only_when_needed() {
        assert_eq!(
            build_identity("mcp-probe", "0.1.0", "abc123", false),
            "mcp-probe@0.1.0+abc123"
        );
        assert_eq!(
            build_identity("mcp-probe", "0.1.0", "abc123", true),
            "mcp-probe@0.1.0+abc123-dirty"
        );
    }
