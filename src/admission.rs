...
pub fn load_startup_admission_config() -> Result<StartupAdmissionConfig, String> {
    ...
    let allow_production_bypass = env_flag("MCP_PROBE_STARTUP_ADMISSION_ALLOW_PROD_BYPASS", false)?;
...
mod tests {
    ...
    fn write_gate_json(path: &Path, runtime: &RuntimeProvenance, status: &str, expires_at: &str) {
        ...
        fs::write(path, serde_json::to_vec(&payload).expect("serialize gate")).expect("write gate");
    }
